...
static void dumpFrame(const uint32_t* p)
{
  ...
  case 1:
    { const uint64_t* pl = reinterpret_cast<const uint64_t*>(p+1);
      printf("BSA_CNTL : TS(%16lx) INI(%16lx) ACT(%16lx) AVG(%16lx) UPD(%16lx) %c\n",
             pl[0],pl[1],pl[2],pl[3],pl[4],m); }
    break;
  case 2:
    { const uint64_t* pl = reinterpret_cast<const uint64_t*>(p+1);
      printf("BSA_CHN LCLS%c chan [x%x]: PID(%16lx) ACT(%16lx) AVG(%16lx) TS(%16lx) UPD(%16lx) %c\n",
             (p[0]&(1<<22)) ? '1':'2',
             (p[0]>>0)&0xff,pl[0],pl[1],pl[2],pl[3],pl[4],m); }
    break;
  default:
    printf("UNKNOWN [x%x]\n", p[0]);
    break;

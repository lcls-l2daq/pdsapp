...
int main(int argc, char** argv) {
  ...
  for(unsigned i=0; i<32; i++) {
    write_reg(&strm, 40, 1<<i);
    unsigned v = read_reg(&strm, 40);
    if (v != (1<<i))
  ...
  for(unsigned i=0; i<10; i++)
    printf("reg[%x] %x\n", i, read_reg(&strm, 4*i));
  for(unsigned i=0x70; i<0x75; i++)
    printf("reg[%x] %x\n", i, read_reg(&strm, 4*i));
  for(unsigned i=0xf0; i<0xf1; i++)
    printf("reg[%x] %x\n", i, read_reg(&strm, 4*i));
  ...
  while(1) {
    ...
    unsigned nerr_b = read_reg(&strm, 20);

    sleep(1);

    unsigned nerr_e = read_reg(&strm, 20);

    printf("nerr: %u [%x,%x]\n",nerr_e-nerr_b, nerr_b, nerr_e);
  }
}
